...
- **工程能力：** 使用 Codex，Claude Code 快速搭建完整 Demo，并通过测试、CI 和数据边界验证产品假设。
- **问题偏好：** 喜欢把模糊需求拆成可观察、可评估、可持续迭代的产品系统。
- **求职方向：** AI 产品经理、AI 应用产品与相关实习机会。

## 经历

- **百度 · 文心一言 · AI产品经理实习** — 参与大模型与 AI Coding 产品评测，负责评测体系设计、多模型测试及问题归因，支持模型选型与产品迭代。
- **百川智能 · AI产品经理实习** — 参与医疗 RAG 知识助手，负责知识库构建与检索问答链路（脱敏复刻版见下方 RAG 项目）。
- **德勤华永会计师事务所（特殊普通合伙） · IT审计实习** — 在工作中积极使用AI探索，帮助团队提高工作效率 （见下方 审计工具 项目）。
